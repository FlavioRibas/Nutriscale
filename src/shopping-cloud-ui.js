...
function selectedRecipeIdsFromDom(recipes){
  ...
  return selectedTitles.map(title=>recipes.find(x=>x.title===title)?.id).filter(Boolean);
}
function syncManualRecipeSelection(userId){
  const recipes=safeJson(localStorage.getItem('nutriscale_recipes'),[]);
  const storedKey=key('nutriscale_shopping_recipe_ids',userId);
  const stored=new Set(safeJson(localStorage.getItem(storedKey),[]));
  const inputs=[...document.querySelectorAll('.manual-recipe-options input[type="checkbox"]')];
  for(const input of inputs){
    const title=input.closest('label')?.querySelector('strong')?.textContent?.trim();
    const recipe=recipes.find(x=>x.title===title);
    if(recipe&&stored.has(recipe.id)&&!input.checked) input.click();
  }
  const current=selectedRecipeIdsFromDom(recipes);
  localStorage.setItem(storedKey,JSON.stringify(current));
  for(const input of inputs){
    if(input.dataset.kpCloudBound==='1') continue;
    input.dataset.kpCloudBound='1';
    input.addEventListener('change',()=>setTimeout(()=>{
      const latest=safeJson(localStorage.getItem('nutriscale_recipes'),[]);
      localStorage.setItem(storedKey,JSON.stringify(selectedRecipeIdsFromDom(latest)));
    },0));
  }
}
function currentBuilder(userId){
  ...
  const recipeIds=selectedRecipeIdsFromDom(recipes);
  for(const recipeId of recipeIds){const r=recipes.find(x=>x.id===recipeId);if(r)picked.push(r);}
...
async function render(){
  ...
  syncManualRecipeSelection(userId);
...
let timer;
const observer=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(()=>{const hasShopping=document.querySelector('.shopping-card');if(hasShopping){syncManualRecipeSelection(localStorage.getItem('nutriscale_current_user_id'));if(!document.querySelector('.kinplate-saved-carts'))render();}if(!hasShopping&&panel){panel.remove();panel=null;}},120);});
