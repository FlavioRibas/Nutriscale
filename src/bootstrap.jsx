...
function patchStorageSync(){
  ...
  Storage.prototype.setItem=function(key,value){
    ...
    if(key==='nutriscale_recipes'){
      try{queueRecipeSync(JSON.parse(value||'[]'));}catch(error){console.error('NutriScale recipe sync failed',error);}
    }
    if(key==='nutriscale_categories'){
      try{syncTagsFromLabels(JSON.parse(value||'[]')).catch(error=>console.error('NutriScale tag sync failed',error));}catch(error){console.error(error);}
    }
  };
}
...
function AuthShell(){
  ...
  if(showImport) return <div className="auth-page"><div className="auth-card premium-card"><div className="auth-brand"><Leaf size={28}/><strong>NutriScale</strong></div><span className="eyebrow">One-time setup</span><h1>{copy.importTitle}</h1><p>{copy.importText(importRecipes.length)}</p><div className="import-summary"><strong>{importRecipes.length}</strong><span>{language==='pt'?'receitas encontradas':'recipes found'}</span></div>{message&&<div className="auth-message">{message}</div>}<button className="primary-button full-button" disabled={importing} onClick={doImport}>{importing?copy.working:copy.importButton}</button><button className="secondary-button full-button" disabled={importing} onClick={skipImport}>{copy.skip}</button><p className="auth-small">Nothing is uploaded until you choose Import.</p></div></div>;
  if(session?.user) return <div className="auth-page"><div className="auth-card premium-card"><p>Loading NutriScale…</p></div></div>;

  return <div className="auth-page"><div className="auth-layout"><section className="auth-welcome"><div className="auth-brand large"><Leaf size={34}/><strong>NutriScale</strong></div><span className="eyebrow">Real Food. A Healthier You. Together.</span><h1>{copy.title}</h1><p>{copy.subtitle}</p><div className="auth-promise"><span>✓</span><div><strong>Private account</strong><p>Your meal plans and private notes stay yours.</p></div></div><div className="auth-promise"><span>✓</span><div><strong>Flexible recipes</strong><p>One recipe can have multiple tags such as Breakfast, Snack, Smoothie and Pre-Workout.</p></div></div></section><section className="auth-card premium-card"><label className="auth-language"><Languages size={17}/><select value={language} onChange={e=>setLanguage(e.target.value)}><option value="en">English</option><option value="pt">Português (BR)</option></select></label><div className="auth-brand mobile"><Leaf size={25}/><strong>NutriScale</strong></div><h2>{language==='pt'?'Entrar':'Sign in'}</h2>{!isSupabaseConfigured&&<div className="auth-message warning">{copy.config}</div>}<form onSubmit={requestLink}><label><span><UserRound size={16}/>{copy.name}</span><input value={name} onChange={e=>setName(e.target.value)} required autoComplete="name" placeholder={language==='pt'?'Seu nome':'Your name'}/></label><label><span><Mail size={16}/>{copy.email}</span><input value={email} onChange={e=>setEmail(e.target.value)} required type="email" autoComplete="email" placeholder="you@example.com"/></label><button className="primary-button full-button" disabled={busy||!isSupabaseConfigured}><LogIn size={18}/>{busy?(language==='pt'?'Enviando…':'Sending…'):copy.send}</button></form>{message&&<div className="auth-message">{message}</div>}<p className="auth-small">For testing, NutriScale uses a secure email magic link. No password is stored by NutriScale.</p></section></div></div>;
}
